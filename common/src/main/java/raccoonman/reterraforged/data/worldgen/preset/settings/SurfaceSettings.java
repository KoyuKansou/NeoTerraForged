...
public record SurfaceSettings(Erosion erosion) {
	...
	public static final Codec<Erosion> CODEC = RecordCodecBuilder.create(instance -> instance.group(
		...
		Codec.FLOAT.optionalFieldOf("screeValue", 0.4F).forGetter((o) -> o.screeValue),
		Codec.FLOAT.optionalFieldOf("desertGradientMin", 0.15F).forGetter((o) -> o.desertGradientMin),
		Codec.INT.optionalFieldOf("desertHeightMin", 10).forGetter((o) -> o.desertHeightMin),
		Codec.INT.optionalFieldOf("desertHeightThreshold", 40).forGetter((o) -> o.desertHeightThreshold)
	...
	public float desertGradientMin;
	public int desertHeightMin;
	public int desertHeightThreshold;
	...
	this(rockVariance, rockMin, dirtVariance, dirtMin, rockSteepness, dirtSteepness, screeSteepness, 0.4F, 0.15F, 10, 40);
        }

        public Erosion(int rockVariance, int rockMin, int dirtVariance, int dirtMin, float rockSteepness, float dirtSteepness, float screeSteepness, float screeValue, float desertGradientMin, int desertHeightMin, int desertHeightThreshold) {
	...
	this.screeValue = screeValue;
	this.desertGradientMin = desertGradientMin;
	this.desertHeightMin = desertHeightMin;
	this.desertHeightThreshold = desertHeightThreshold;
        }

        public Erosion copy() {
	return new Erosion(this.rockVariance, this.rockMin, this.dirtVariance, this.dirtMin, this.rockSteepness, this.dirtSteepness, this.screeSteepness, this.screeValue, this.desertGradientMin, this.desertHeightMin, this.desertHeightThreshold);
        }
    }
}
