...
public class RTFCellSamplerNode extends DelegateNode {

    private final int fieldOrdinal;

    public RTFCellSamplerNode(CellSampler sampler) {
        super(sampler);
        this.fieldOrdinal = sampler.field().ordinal();
    }

    public RTFCellSamplerNode(CellSampler.CacheChunk sampler) {
        super(sampler);
        this.fieldOrdinal = -1;
    }

    public RTFCellSamplerNode(CellSampler.CacheChunk sampler, int fieldOrdinal) {
        super(sampler);
    ...
    public String doCLGen(OpenCLGen.Context context) {
        if (this.fieldOrdinal < 0) {
            throw new UnsupportedOperationException("CellSampler.CacheChunk field unknown. Falling back to CPU.");
        }

        int dataOffset = context.allocGlobalDynamicData(RTFOpenCLMarkers.MARKER_rtf_cellSampler);
        RTFOpenCLMarkers.setCellSamplerDataOffset(dataOffset);

        // Bilinear interpolation between biome-resolution samples.
        // CellSampler data is precomputed at QuartPos (biome) resolution.
        // Without this, terrain appears blocky at 4x4 block intervals and looks like DH LoDs
        ...
        return "if (ctx.rw_data) {\n" +
                ...
                "        int field = " + this.fieldOrdinal + ";\n" +
                "        double v00 = rtf_data[field * biomeW * biomeH + ix0 * biomeH + iz0];\n" +
                "        double v10 = rtf_data[field * biomeW * biomeH + ix1 * biomeH + iz0];\n" +
                "        double v01 = rtf_data[field * biomeW * biomeH + ix0 * biomeH + iz1];\n" +
                "        double v11 = rtf_data[field * biomeW * biomeH + ix1 * biomeH + iz1];\n" +
